...
The script deliberately downloads raw inputs without altering them. Processing is a
separate stage. Failed optional providers are recorded rather than replaced with
synthetic data.
"""
...
def record(source: str, status: str, path: Path | None, url: str, **extra: Any) -> None:
    row = {"source": source, "status": status, "url": url,
...
def coolr(layer: str, source: str, filename: str) -> bool:
    base = "https://maps.gpm.nasa.gov/landslides/arcgis/rest/services/Landslides/" + layer + "/FeatureServer/0/query"
    ...
    while True:
        params={"where":"1=1", "outFields":"*", "returnGeometry":"true", "f":"geojson",
                ...
                "geometry":f"{AOI['min_lon']},{AOI['min_lat']},{AOI['max_lon']},{AOI['max_lat']}",
    ...
    record(source,"downloaded",out,base,records=len(features),spatial_filter=AOI)
    return True


def usgs() -> bool:
    url="https://earthquake.usgs.gov/fdsnws/event/1/query"
    params={"format":"geojson","starttime":"2000-01-01","endtime":datetime.now(timezone.utc).date().isoformat(),
            "minmagnitude":2.5,"minlatitude":AOI["min_lat"],"maxlatitude":AOI["max_lat"],
            "minlongitude":AOI["min_lon"],"maxlongitude":AOI["max_lon"],"orderby":"time-asc","limit":20000}
    try:
        r=requests.get(url,params=params,headers=HEADERS,timeout=180); r.raise_for_status()
        out=RAW/"seismic"/"usgs_earthquakes_2000_present.geojson"; out.parent.mkdir(parents=True,exist_ok=True); out.write_bytes(r.content)
...
def main() -> int:
    print("Acquiring public NER-SLIDE V6 datasets...")
    ...
    usgs()
    download("https://download.geofabrik.de/asia/india/north-eastern-zone-latest.osm.pbf", RAW/"infrastructure"/"north-eastern-zone-latest.osm.pbf", "osm_ne_india", timeout=3600)
    download("https://data.hydrosheds.org/file/HydroBASINS/standard/hybas_as_lev01-12_v1c.zip", RAW/"hydrology"/"hydrobasins_as_lev01-12_v1c.zip", "hydrobasins_asia", timeout=3600)
    download("https://data.hydrosheds.org/file/HydroRIVERS/standard/HydroRIVERS_v10_as_shp.zip", RAW/"hydrology"/"hydrorivers_asia.zip", "hydrorivers_asia", timeout=3600)
    print("Public acquisition stage complete. See manifests/acquisition_manifest.jsonl")
    return 0
